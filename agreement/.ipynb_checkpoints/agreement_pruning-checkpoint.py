...
def main(args):
    ...
    full_model_target_log_probs = torch.load(f'full_model_results/{args.task_name}_target_log_probs.pt')
    full_model_pred_labels = torch.load(f'full_model_results/{args.task_name}_pred_labels.pt')
    agreement_data_dict = prepare_data_for_clm(raw_agreement_ds, full_model_target_log_probs, full_model_pred_labels)
    agreement_ds = AgreementDataset(agreement_data_dict)
    
    n_train = int(0.8*len(agreement_ds))
    n_test = len(agreement_ds) - n_train
    agreement_ds_train, agreement_ds_test = torch.utils.data.random_split(agreement_ds, [n_train, n_test])
    ...
    if args.resume_epoch_w > 0:
        weight_mask_logits = torch.load(join(args.mask_dir, f'mask_logits_dict_weight_{args.task_name}_weight_{args.resume_epoch_w}_edge_0.pt'))
        circuit_gpt.load_pretrained_weight_mask(weight_mask_logits)
    # if args.resume_epoch_e > 0:
    #     edge_mask_logits = torch.load(join(args.mask_dir, f'mask_logits_dict_edge_{args.task_name}_weight_{args.train_epochs_weight}_edge_{args.resume_epoch_e}.pt'))
    ...
    # for n, mask_logits in circuit_gpt.mask_logits_dict_weight.items():
    #     mask_logits.to(device)
    # for n, mask_logits in circuit_gpt.mask_logits_dict_edge.items():
    #     mask_logits.to(device)
    # print(f'edge logits: {circuit_gpt.mask_logits_dict_edge}')
        ...
        for epoch in tqdm(range(args.train_epochs_weight)):
            ...
            if eval_results_faith['acc'] > 0.95 and eval_results_faith['weight_density'] < 0.05:
                torch.save(
                    ...
                    join(args.mask_dir, f'mask_logits_dict_weight_{args.task_name}_weight_{epoch}_edge_0.pt')
                )
    else:
    ...
    if args.prune_edge:
        ...
        for epoch in tqdm(range(args.train_epochs_edge)):
            ...
            for batch in train_dl:
                ...
                # print(f'faith_loss_edge: {faith_loss_edge}')
                ...
                # print(f'complete_loss_edge: {complete_loss_edge}')
            ...
            if eval_results_faith['acc'] > 0.95 and eval_results_faith['edge_density'] < 0.05:
                torch.save(
                    ...
                    join(args.mask_dir, f'mask_logits_dict_edge_{args.task_name}_weight_{n_epoch_w}_edge_{epoch}.pt')
                )
            

if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--mask_dir', default='/home/leiyu/scratch/circuit-discovery/mask_logits/', type=str)
