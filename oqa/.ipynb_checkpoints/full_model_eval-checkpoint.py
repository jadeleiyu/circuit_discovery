...
def main(args):
    ...
    pararel_rels = PARAREL_RELS.split(' ')

    #########################
    for rel_id in tqdm(pararel_rels):        
        data = pararel_rel_data[rel_id]
        ...
        if len(ds_dict['answer']) > 0:
            ...
            print(f'pararel relation: {rel_id}')
            ...
            torch.save(full_model_target_log_probs, f'full_model_results/target_log_probs_{rel_id}.pt')
            torch.save(full_model_pred_labels, f'full_model_results/pred_labels_{rel_id}.pt')
            torch.save(answer_vocab_idx, f'full_model_results/answer_vocab_idx_{rel_id}.pt')
            pickle.dump(ds_dict, open(join(args.data_dir, f'pararel_ds_dict_{rel_id}.p'), 'wb'))
