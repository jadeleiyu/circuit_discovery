...
def main(args):
    rel_id = PARAREL_RELS[args.job_id]
    
    logger = logging.getLogger(__name__)
    logger.setLevel(logging.DEBUG)
    logging.basicConfig(format='%(message)s', filename=f'log/{rel_id}-{args.exp_name}.log')
    ...
    # load OQA data
    ds_dict = pickle.load(open(join(args.data_dir, f'pararel_ds_dict_{rel_id}.p'), 'rb'))
    full_model_target_log_probs = torch.load(f'full_model_results/target_log_probs_{rel_id}.pt')
    full_model_pred_labels = torch.load(f'full_model_results/pred_labels_{rel_id}.pt')
    answer_vocab_idx = torch.load(f'full_model_results/answer_vocab_idx_{rel_id}.pt')
    ...
    if args.resume_epoch_w > 0:
        weight_mask_logits = torch.load(join(args.results_dir, f'mask_logits_dict_weight_oqa_{rel_id}_weight_{args.resume_epoch_w}_edge_0.pt'))
    ...
    logger.info(f'task name: oqa')
    logger.info(f'pararel relation: {rel_id}')
    logger.info(f'prune weight: {args.prune_weight}')
    logger.info(f'use weight masks: {args.use_weight_masks}')
    logger.info(f'prune edge: {args.prune_edge}')
    logger.info(f'optimize weight completeness: {args.lambda_complete_weight_init > 0}')
    logger.info(f'optimize edge completeness: {args.lambda_complete_edge_init > 0}')
    ...
    if args.prune_weight:
        ...
        for epoch in tqdm(range(args.train_epochs_weight)):
            ...
            if eval_results_faith['acc'] > 0.95 and eval_results_faith['weight_density'] < 0.05:
                logger.info(f'saving weight mask logits for epoch {epoch}')
                mask_logits_dict_weight = {k: v.detach().cpu() for k,v in circuit_gpt.mask_logits_dict_weight.items()}
                torch.save(
                    mask_logits_dict_weight,
                    join(args.results_dir, f'mask_logits_dict_weight_oqa_{rel_id}_weight_{epoch}_edge_0.pt')
                )
        # save final weight masks            
        logger.info(f'saving weight mask logits for epoch {epoch}')
        mask_logits_dict_weight = {k: v.detach().cpu() for k,v in circuit_gpt.mask_logits_dict_weight.items()}
        torch.save(
            mask_logits_dict_weight,
    ...
    if args.prune_edge:
        ...
        for epoch in tqdm(range(args.train_epochs_edge)):
            ...
            if eval_results_faith['acc'] > 0.95 and eval_results_faith['weight_density'] < 0.05:
                logger.info(f'saving edge mask logits for epoch {epoch}')
                mask_logits_dict_edge = {k:v.detach().cpu() for k,v in circuit_gpt.mask_logits_dict_edge.items()}
                torch.save(
                    mask_logits_dict_edge,
                    join(args.results_dir, f'mask_logits_dict_edge_oqa_{rel_id}_weight_{n_epoch_w}_edge_{epoch}.pt')
                )
        # save final edge masks
        logger.info(f'saving edge mask logits for epoch {epoch}')
        mask_logits_dict_edge = {k:v.detach().cpu() for k,v in circuit_gpt.mask_logits_dict_edge.items()}
        torch.save(
            mask_logits_dict_edge,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--exp_name', default='weight-only', type=str)
    # parser.add_argument('--rel_id', default='P103', type=str)
