...
def prepare_ioi_data_for_clm(ioi_prompts, full_model_target_log_probs=None, full_model_pred_labels=None):
    sents = []
    targets = []
    ...
    if full_model_target_log_probs is not None:
        data_dict['full_model_target_log_probs'] = full_model_target_log_probs
    if full_model_pred_labels is not None:
        data_dict['full_model_pred_labels'] = full_model_pred_labels
    
    return data_dict


class IOIFullModelDataset(Dataset):
...
class IOICircuitDataset(Dataset):
...
def prepare_batch_inputs(batch, tokenizer):
    ...
    if 'full_model_target_log_probs' in batch:
        return {
